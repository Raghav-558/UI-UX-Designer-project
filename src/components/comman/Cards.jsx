  ...
  return (
    <div id="work" className="bg-black">
      <div className="max-w-[1140px] mx-auto px-5 pt-16 max-md:pt-10 pb-16 max-md:pb-10">
        <div
          data-aos="fade-down"
          className="text-left flex items-center gap-16 max-sm:flex-col max-sm:gap-3"
        >
          <h2 className="font-eczar font-semibold text-fs-2xl max-md:text-3xl text-white">
            Featured Work
          </h2>
          <div className="font-worksans font-semibold text-xl max-md:text-base max-sm:px-4 max-sm:py-3 rounded-md max-sm:rounded-sm py-4 px-6 border-4 border-white text-white">
          ...
          {CARDS.slice(0, show).map((obj, index) => (
            <div key={index} className="w-full md:w-1/2 lg:w-1/3 px-3">
              <a href="#">
                <div
                  data-aos="zoom-in"
                  className="bg-white rounded-md p-2.5 mb-6 group/card"
                >
                  <div className="overflow-hidden rounded-md">
                    <img
                      src={obj.img}
                      alt={obj.title}
                      className="transition-all duration-300 group-hover/card:scale-[1.02] w-full h-[300px] object-cover"
                    />
                  </div>
                  <div className="p-3">
                    <h3 className="font-worksans font-semibold text-2xl max-sm:text-xl">
                      {obj.title}
                    </h3>
                  </div>
                </div>
